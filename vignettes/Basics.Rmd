...
Alternatively, one can generate this figure through `plot` and specify `plot_type = "function"`:

```{r, fig.height=4, fig.width=4}
plot(fash_fit, selected_unit = detected_indices[1], plot_type = "function")
```
...
With the posterior, we might also be interested in testing whether $H_0: \mathcal{F}(\beta_j(t)) = 0$ for some functional $\mathcal{F}$.
...
```{r}
bar_colors <- ifelse(labels == "A", "gray", "blue")
barplot(lfsr_result$lfsr, col = bar_colors, xlab = "eQTL index", ylab = "LFSR")
legs <- c("Category A", "Category B/C")
legend("topleft", legend = legs, fill = c("gray", "blue"))
```
