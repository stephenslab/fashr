...
Using these results we can visualize, for example, the posterior mean
effect estimate:
...
One can generate a more detailed figure of the posterior simply through `plot`
and specify `plot_type = "function"`. 
By default, the figure shows the posterior mean effect (solid line) along with 95% credible bands (shaded area), as well as the raw data points as black dots together with their (+-2) standard errors (black error bars).
As a comparison, we add the true effect (that is, the effect used to simulate the data) to the figure as a dashed line.
...
Visualize the posterior of the first detected nonlinear dynamic
eQTL. The true effect is shown as a dashed line.
